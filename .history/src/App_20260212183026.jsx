...
function App() {
  ...
  // ❌ NOT be called inside if statements
  // ❌ NOT be called inside nested functions
  
  // Why?
  
  // Because React tracks hooks in order.
  
  // If the order changes → React breaks.



  function column ({status, tasks, childern}) {
    const {setNodeRef} = useDroppable({id: status,});
  }

  function Task({task, children}) {
    const {attributes, listeners, setNodeRef, transform} = use
   }
